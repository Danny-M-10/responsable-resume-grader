...
FAMILY_NAME=$(python3 -c "import json; print(json.load(open('task-definition.json'))['family'])" 2>/dev/null || echo "$TASK_DEF_NAME")
...
if [ ! -z "$TARGET_GROUP_ARN" ] && [ "$TARGET_GROUP_ARN" != "None" ]; then
    ...
    # AWS ELBv2 uses healthcheck.path (with dot) as the attribute key
    if aws elbv2 modify-target-group-attributes \
        --target-group-arn "$TARGET_GROUP_ARN" \
        --attributes "Key=healthcheck.path,Value=/health" \
        --region $AWS_REGION \
        > /dev/null 2>&1; then
